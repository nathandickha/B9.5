...
const SPA_WALL_ALIGN_BRIDGE_OVERLAP = 0.02; // tiny bridge only, to close the seam without creating a pool-side channel
...
function getSpaChannelMargins(spaGroup, poolGroup = null) {
  ...
  if (snapSide === 'left') {
    ...
    margins.minY = channel;
    margins.maxY = channel;
    if (snapVariant === 'inner-wall-align') margins.maxX = bridgeToPoolWall.left + SPA_WALL_ALIGN_BRIDGE_OVERLAP;
    return margins;
  }
  if (snapSide === 'right') {
    margins.maxX = channel;
    margins.minY = channel;
    margins.maxY = channel;
    if (snapVariant === 'inner-wall-align') margins.minX = bridgeToPoolWall.right + SPA_WALL_ALIGN_BRIDGE_OVERLAP;
    return margins;
  }
  if (snapSide === 'front') {
    margins.minY = channel;
    margins.minX = channel;
    margins.maxX = channel;
    if (snapVariant === 'inner-wall-align') margins.maxY = bridgeToPoolWall.front + SPA_WALL_ALIGN_BRIDGE_OVERLAP;
    return margins;
  }
  if (snapSide === 'back') {
    margins.maxY = channel;
    margins.minX = channel;
    margins.maxX = channel;
    if (snapVariant === 'inner-wall-align') margins.minY = bridgeToPoolWall.back + SPA_WALL_ALIGN_BRIDGE_OVERLAP;
    return margins;
  }
...
function getForcedSpaChannelFullSpanSides(spaGroup) {
  ...
  // Only force the bridge side to span the full opening when using the new
  // outer wall-align snap. The middle snap keeps its existing behaviour.
  if (snapVariant !== 'inner-wall-align') return new Set();

  if (snapSide === 'left') return new Set(['maxX']);
  if (snapSide === 'right') return new Set(['minX']);
  if (snapSide === 'front') return new Set(['maxY']);
  if (snapSide === 'back') return new Set(['minY']);
  return new Set();
}
...
function updateSpaChannelMeshes(ground, poolGroup, spaGroup) {
  ...
  const forcedFullSpanSides = getForcedSpaChannelFullSpanSides(spaGroup);
  const getSideIntervals = (_sideKey, axisAlong, axisAcross, stripCenter, alongMin, alongMax) => {
    if (forcedFullSpanSides.has(_sideKey)) return [[alongMin, alongMax]];
