...
function getSpaChannelMargins(spaGroup, poolGroup = null) {
  ...
  if (snapSide === 'left') {
    ...
    margins.minY = channel;
    margins.maxY = channel;
    if (snapVariant === 'inner-wall-align') margins.maxX = bridgeToPoolWall.left;
    return margins;
  }
  if (snapSide === 'right') {
    margins.maxX = channel;
    margins.minY = channel;
    margins.maxY = channel;
    if (snapVariant === 'inner-wall-align') margins.minX = bridgeToPoolWall.right;
    return margins;
  }
  if (snapSide === 'front') {
    margins.minY = channel;
    margins.minX = channel;
    margins.maxX = channel;
    if (snapVariant === 'inner-wall-align') margins.maxY = bridgeToPoolWall.front;
    return margins;
  }
  if (snapSide === 'back') {
    margins.maxY = channel;
    margins.minX = channel;
    margins.maxX = channel;
    if (snapVariant === 'inner-wall-align') margins.minY = bridgeToPoolWall.back;
    return margins;
  }
